...
print('Welcome to the python-file-explorer\n')

# Stores every drive name connected on computer in  list.
drives_NAME = [chr(x) + ':' for x in range(65, 90) if os.path.exists(chr(x) + ':')]


# Name each folder and files present in the current working path
def listDirectories():
    """
do the all list of directories and print it.
    """
...
while True:
    ...
    if result == '1':
        ...
        print('Drives: ')
        for x in range(len(drives_NAME)):
            print(str(5 + x) + '. ' + drives_NAME[x])
    ...
    if result == '2':
        ...
        print('Drives: ')
        for x in range(len(drives_NAME)):
            print(str(1 + x) + '. ' + drives_NAME[x])

        while True:
            inp = input("\nEnter your Choice: ")

            if inp in drives_NAME:
                os.chdir(inp + '\\')
                break
            else:
        ...
        while True:
            ...
            if res in os.listdir(os.getcwd()):
                ...
                if os.path.isfile(res):
                    ...
                    os.chdir(res)
            # Exit command to exit from loop
            elif res == 'exitManager':
                sys.exit(0)
            # Back command to go up one directory
